...
if ($query) {
	...
	$levels = query("SELECT $userfields l.id,l.title FROM levels l JOIN users u ON l.author = u.id WHERE l.title LIKE CONCAT('%', ?, '%') AND l.locked = 0 ORDER BY l.id DESC $limit",
		[$query]);
